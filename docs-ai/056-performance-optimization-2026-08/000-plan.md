...
| #658 | Share transcript directory walks and reuse derived scoring inputs | Reviewed and integrated through #662: one-second walk reuse cannot cross the narrow sole-candidate confirmation retry; grapheme-count and suffix reuse remain score-equivalent | `d5825e55` |
| #659 | Memoize agent working-directory resolution across renders | Reviewed and integrated through #663: repository and symlink revalidation clear the bounded memo, while one row batch validates the repository signature once | `549b1c20` |
| #660 | Coalesce Active Agents emissions driven only by animated pane titles | Reviewed and integrated through #664: semantic changes remain immediate, poll-driven trailing delivery is retained, obsolete pending frames are discarded, and Profile attribution is protected | `80f0b11c` |
| #661 | Record the profiling wave and add reusable measurement scripts | Reviewed and integrated through #665: process selection, failure semantics, numeric inputs, output privacy, and the durable record are hardened against ambiguous or misleading captures | `0cc92c82` |
