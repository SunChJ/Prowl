...
macOS instead of arranging any of them manually. On macOS 26, removing the
last sidebar toolbar item also changes titlebar ownership and moves the traffic
lights out of the sidebar well, so Settings intentionally keeps the standard
system toggle. A focused-scene close action lets the shared Close Window command
use SwiftUI's `dismiss` for Settings, so Cmd-W continues to work even when the
main-window terminal close actions are currently registered.
