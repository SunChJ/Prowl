...
- `supacode/Features/Settings/Views/SettingsView.swift` — uses the normal
  ...
  SwiftUI/macOS. Removing the toggle was tested but changes the macOS 26
  titlebar/sidebar relationship, leaving the traffic lights outside the
  sidebar well, so that workaround was not retained.
